function exchangeclone.shear_action(itemstack, player, center)
    ...
    local leaves = core.get_item_group(start_node.name, "leaves") > 0
    ...
    local nodes = core.find_nodes_in_area(pos1, pos2, grouped)
        ...
        if leaves or (not leaves and node.name ~= "mcl_flowers:double_grass_top") then
            if core.is_protected(pos, player:get_player_name()) then
                ...
                if node.name:sub(1,18) == "mcl_ocean:seagrass" then
                    core.set_node(pos, {name="mcl_core:sand"})
                end
            end
        end
    end
...
local shears_rightclick = function(itemstack, player, pointed_thing)
    ...
    if (pointed_thing.type == "node") and not player:get_player_control().sneak  then
        ...
        elseif core.get_item_group(node.name, "shearsy") > 0
...
core.register_tool("ec_tools:dark_matter_shears", {
...
exchangeclone.set_charge_type("ec_tools:dark_matter_shears", "dark_matter")

core.register_tool("ec_tools:red_matter_shears", {
...
exchangeclone.set_charge_type("ec_tools:red_matter_shears", "red_matter")
...
local old_sheep_function = core.registered_entities["mobs_mc:sheep"].on_rightclick
if old_sheep_function then
    core.registered_entities["mobs_mc:sheep"].on_rightclick = function(self, clicker)
        local item = clicker:get_wielded_item()
        local item_name = item:get_name()
        if special_shears[item_name] and not self.gotten and not self.child then
            local pos = self.object:get_pos()
            local chance = 30 --percent
            if item_name == "ec_tools:red_matter_shears" or item_name == "ec_tools:red_katar" then chance = 60 end
            if math.random(1, 100) <= chance then
                local new_sheep = core.add_entity(pos, "mobs_mc:sheep"):get_luaentity() --clone the sheep
            ...
            local max_wool = 8
            if item_name == "ec_tools:red_matter_shears" or item_name == "ec_tools:red_katar" then max_wool = 12 end
            core.add_item(pos, ItemStack(colors[self.color][1].." "..math.random(1,max_wool))) --normally 3
...
local old_mooshroom_function = core.registered_entities["mobs_mc:mooshroom"].on_rightclick
if old_mooshroom_function then
    core.registered_entities["mobs_mc:mooshroom"].on_rightclick = function(self, clicker)if self:feed_tame(clicker, 1, true, false) then return end
        ...
        if special_shears[item_name] then
            ...
            core.sound_play("mcl_tools_shears_cut", {pos = pos}, true)
            local amount = 8
            if item_name == "ec_tools:red_matter_shears" or item_name == "ec_tools:red_katar" then
                amount = 12
            end
            if self.base_texture[1] == "mobs_mc_mooshroom_brown.png" then
                core.add_item(player_pos, "mcl_mushrooms:mushroom_brown "..amount)
            else
                core.add_item(player_pos, "mcl_mushrooms:mushroom_red "..amount)
            end

            local chance = 30 --percent
            if item_name == "ec_tools:red_matter_shears" or item_name == "ec_tools:red_katar" then chance = 60 end
            if math.random(1, 100) <= chance then
                local new_mooshroom = core.add_entity(pos, "mobs_mc:mooshroom"):get_luaentity() --clone the mooshroom
                for attribute, value in pairs(self) do
                    if attribute ~= "object" then
            ...
            self.object:remove()
            local cow = core.add_entity(pos, "mobs_mc:cow") --[[@as core.ObjectRef]]
            cow:set_yaw(oldyaw)
        else
    ...
    local old_honey_harvest = core.registered_items["mcl_beehives:bee_nest_5"].on_rightclick
    for _, itemstring in pairs({"mcl_beehives:bee_nest_5", "mcl_beehives:beehive_5"}) do
        core.registered_items[itemstring].on_rightclick = function(pos, node, player, itemstack, pointed_thing)
...
--local old_dispenser_function = core.registered_items["mcl_dispensers:dispenser"].mesecons.effector.action_on
local new_dispenser_function = function(pos, node)
    ...
    if #stacks >= 1 then
        ...
        if special_shears[iname] then
            for _, obj in pairs(core.get_objects_inside_radius(droppos, 1)) do
                ...
                if entity and not entity.child and not entity.gotten then
                    ...
                    if entname == "mobs_mc:sheep" then
                        local max_wool = 8
                        if iname == "ec_tools:red_matter_shears" or iname == "ec_tools:red_katar" then max_wool = 12 end
                        core.add_item(entity_pos, entity.drops[2].name.." "..math.random(1,max_wool)) --normally 3
                        if not entity.color then
                            entity.color = "unicolor_white"
                        end
                        local chance = 30 --percent
                        if iname == "ec_tools:red_matter_shears" or iname == "ec_tools:red_katar" then chance = 60 end
                        if math.random(1, 100) <= chance then
                            local new_sheep = core.add_entity(entity_pos, "mobs_mc:sheep"):get_luaentity() --clone the sheep
                    ...
                    elseif entname == "mobs_mc:mooshroom" then
                        local amount = 8
                        if iname == "ec_tools:red_matter_shears" or iname == "ec_tools:red_katar" then
                            amount = 12
                        end
                        if entity.base_texture[1] == "mobs_mc_mooshroom_brown.png" then
                            core.add_item(droppos, "mcl_mushrooms:mushroom_brown "..amount)
                        else
                            core.add_item(droppos, "mcl_mushrooms:mushroom_red "..amount)
                        end

                        local chance = 30 --percent
                        if iname == "ec_tools:red_matter_shears" or iname == "ec_tools:red_katar" then chance = 60 end
                        if math.random(1, 100) <= chance then
                            local new_mooshroom = core.add_entity(entity_pos, "mobs_mc:mooshroom"):get_luaentity() --clone the mooshroom
                            for attribute, value in pairs(entity) do
                                if attribute ~= "object" then
                                    new_mooshroom[attribute] = value
                                end
                    ...
                    if used then
                        ...
                        entity.gotten = true
                        core.sound_play("mcl_tools_shears_cut", { pos = entity_pos }, true)
...
for _, itemstring in pairs({"mcl_dispensers:dispenser", "mcl_dispensers:dispenser_up", "mcl_dispensers:dispenser_down"}) do
    --core.registered_items[itemstring].mesecons.effector.action_on = new_dispenser_function
end
...
local old_snowman_function = core.registered_entities["mobs_mc:snowman"].on_rightclick
if old_snowman_function then
    core.registered_entities["mobs_mc:snowman"].on_rightclick = function(self, clicker)
        local item = clicker:get_wielded_item()
        local item_name = item:get_name()
...
core.register_craft({
    output = "ec_tools:dark_matter_shears",
    recipe = {
...
core.register_craft({
    output = "ec_tools:red_matter_shears",
    recipe = {
        {"", "ec_matter:red_matter"},
        {"ec_tools:dark_matter_shears", ""},
    }
})

core.register_alias("exchangeclone:dark_matter_shears", "ec_tools:dark_matter_shears")
core.register_alias("exchangeclone:red_matter_shears", "ec_tools:red_matter_shears")