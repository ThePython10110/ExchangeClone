...
core.register_node("ec_emc_link:emc_link", {
...
if exchangeclone.pipeworks then
    ...
    core.override_item("ec_emc_link:emc_link", {
        tube = {
            input_inventory = "dst",
            ...
            can_insert = function(pos, node, stack, direction)
                local meta = core.get_meta(pos)
                local inv = meta:get_inventory()
                local listname = get_list(direction)
                return (allow_metadata_inventory_put(pos, listname, 1, stack) > 0) and inv:room_for_item(listname, stack)
            end,
        },
...
core.register_craft({
    output = "ec_emc_link:emc_link",
    recipe = {
        {exchangeclone.itemstrings.obsidian, exchangeclone.itemstrings.diamond, exchangeclone.itemstrings.obsidian},
        {exchangeclone.itemstrings.diamond, "ec_chests:alchemical_chest", exchangeclone.itemstrings.diamond},
        {exchangeclone.itemstrings.obsidian, exchangeclone.itemstrings.diamond, exchangeclone.itemstrings.obsidian},
    }
})