...
local function place_liquid(itemstack, player, pointed_thing)
    ...
    local liquid = itemstack:get_name() == "ec_magic_items:volcanite_amulet" and "lava" or "water"
    ...
    if pointed_thing and pointed_thing.type == "node" then
        ...
        if (not exchangeclone.mcl) or core.settings:get_bool("mcl_buckets_use_select_box", false) then
            ...
            core.add_entity(
                ...
                "ec_magic_items:projectile", core.serialize({
...
core.register_entity("ec_magic_items:projectile", {
    initial_properties = {
        hp_max = 1,
        physical = true,
    ...
    on_step = function(self, dtime, moveresult)
        for _, collision in ipairs(moveresult.collisions) do
            ...
            elseif collision.type == "object" then
                local obj = collision.object --[[@as core.ObjectRef]]
...
core.register_tool("ec_magic_items:evertide_amulet", {
...
core.register_tool("ec_magic_items:volcanite_amulet", {
    ...
    if hp_change < 0 then
        if reason.type == "drown" then
            ...
            for i = 1, hotbar_max do
                ...
                if stack:get_name() == "ec_magic_items:evertide_amulet" then
                    return 0
                end
            end
...
core.register_craft({
    output = "ec_magic_items:evertide_amulet",
    recipe = {
...
core.register_craft({
    output = "ec_magic_items:volcanite_amulet",
    recipe = {
