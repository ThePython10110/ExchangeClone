...
local function check_for_furnaces(pos, set_furnace, start)
	...
	for _, check_pos in pairs(exchangeclone.neighbors) do
        ...
        local furnace = core.get_item_group(node.name, "exchangeclone_furnace")
		if furnace > 0 then
            found = true
            if start then
                local timer = core.get_node_timer(new_pos)
			...
			if set_furnace ~= nil then
                local meta = core.get_meta(new_pos)
...
local function on_timer(pos, elapsed)
    ...
    local light = core.get_natural_light(above)
    ...
    if light and light >= 14 then
        ...
        if using_star then
            ...
            local stored = exchangeclone.get_star_emc(inv, "main", 1) --[[@as number]]
            ...
            if placer and placer ~= "" then
                local player = core.get_player_by_name(placer)
                if player then
...
for i = 2, #collectors do
    ...
    local previous = "ec_collectors:"..collectors[i-1]:lower().."_collector"
    exchangeclone.register_energy_collector(
        "ec_collectors:"..codified,
        ...
        "exchangeclone_"..codified..".png",
        {
            {
                previous,
                "ec_matter:"..collector:lower().."_matter"
            },
            "shapeless"
        },
        5+i
    )
    core.register_alias("exchangeclone:energy_collector_mk"..i, "ec_collectors:"..codified)
    core.register_alias("exchangeclone:"..codified, "ec_collectors:"..codified)
end

-- This is apparently the fourth name I've had for this item. Fun.
core.register_alias("exchangeclone:basic_collector", "ec_collectors:basic_collector")
core.register_alias("exchangeclone:energy_collector", "ec_collectors:basic_collector")
core.register_alias("exchangeclone:energy_collector_mk1", "ec_collectors:basic_collector")