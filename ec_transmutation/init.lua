...
local function handle_inventory(player, inventory, to_list)
    ...
    if to_list == "learn" then
        ...
        if itemstring == "ec_transmutation:tome_of_knowledge" then
            list = {}
            local i = 0
            ...
            local max_count = math.floor((exchangeclone.emc_limit - player_emc)/individual_emc_value)
...
local function check_for_table(player, inv)
    ...
    if player:get_wielded_item():get_name() ~= "ec_transmutation:transmutation_tablet"
    and not core.find_node_near(player:get_pos(), range, "ec_transmutation:transmutation_table", true) then return false end
    return true
end
...
local function allow_inventory_action(player, stack, to_list, count, move, inventory)
    ...
    elseif to_list == "learn" then
        if stack:get_name() == "ec_transmutation:tome_of_knowledge" then return count end
...
core.register_tool("ec_transmutation:transmutation_tablet", {
...
core.register_node("ec_transmutation:transmutation_table", {
...
core.register_tool("ec_transmutation:tome_of_knowledge", {
...
core.register_craft({
    output = "ec_transmutation:transmutation_table",
    recipe = {
        {exchangeclone.itemstrings.obsidian, exchangeclone.itemstrings.stone, exchangeclone.itemstrings.obsidian},
        {exchangeclone.itemstrings.stone, "ec_phil:philosophers_stone", exchangeclone.itemstrings.stone},
        {exchangeclone.itemstrings.obsidian, exchangeclone.itemstrings.stone, exchangeclone.itemstrings.obsidian}
    },
...
core.register_craft({
    output = "ec_transmutation:transmutation_tablet",
    recipe = {
        {"ec_matter:dark_matter_block", exchangeclone.itemstrings.stone, "ec_matter:dark_matter_block"},
        {exchangeclone.itemstrings.stone, "ec_transmutation:transmutation_table", exchangeclone.itemstrings.stone},
        {"ec_matter:dark_matter_block", exchangeclone.itemstrings.stone, "ec_matter:dark_matter_block"}
    },
})
...
if core.settings:get_bool("exchangeclone.allow_crafting_alchemical_tome", false) then
    core.register_craft({
        output = "ec_transmutation:tome_of_knowledge",
        recipe = {
            {"", exchangeclone.itemstrings.book, ""},
            {"ec_stars:klein_star_omega", "ec_phil:philosophers_stone", "ec_stars:klein_star_omega"},
...
core.register_craft_predict(function(itemstack, player, old_craft_grid, craft_inv)
    if itemstack == ItemStack("ec_transmutation:tome_of_knowledge") then
...
core.register_alias("exchangeclone:alchemical_tome", "ec_transmutation:tome_of_knowledge")
core.register_alias("exchangeclone:tome_of_knowledge", "ec_transmutation:tome_of_knowledge")
core.register_alias("exchangeclone:transmutation_table", "ec_transmutation:transmutation_table")
core.register_alias("exchangeclone:transmutation_tablet", "ec_transmutation:transmutation_tablet")